...
void dispensing_sequence(int* internalPillContainers){ //stage 1 - set up
    ...
    TA0CCTL0 |= CCIE; //enable interrupt for 1.5s timer that traverses dispensing stages
}
...
void move_linear_actuator(int dir){
    ...
    if (dir == 1){  //linear actuator was told to go up
...
void dispense_pill(){
    if (pillsToDispense > 0){
        ...
        if (servo_toggle == 0){ //it's in 0 deg
...
void recalculate_variables(void){
    ...
    if (currentDirection < 0){ //anti-clockwise
    ...
    } else { //no more pills to dispense, reset pill containers to their original positions for the next alarm
...
__interrupt void stages_ISR(void){
    ...
    TA0CCTL0 &= ~CCIE; //disable interrupt, gets reactivated in each of the other functions
...
__interrupt void stepper_ISR(void){
    ...
    if (steps > 0){
        ...
        } else { //if reset == 1, we're done dispensing pills, don't do anything else
            stage = 8; //go to last stage, reset all values
        }
        TA0CCTL0 |= CCIE; //enable interrupt for 1s timer that traverses dispensing stages
    }
}
